...
result=$(
  curl -s \
    ...
    "https://twitter.com/i/profiles/show/$monitoredTwitterUser/timeline/tweets?include_available_features=1&include_entities=1&include_new_items_bar=true"
)
jq=$(echo $result | grep -o '{"min_position":.*')

#totalPosts=`echo "$jq" | jq .new_latent_count` //I will not use it, but maybe can help you
